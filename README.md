...
- 支持文本和可选图片输入的 Chat Completions 兼容转换；文本模型默认拒绝图片并返回清晰错误，视觉模型需在预设中启用“支持图片输入”。
...
- [Responses 协议兼容说明 / Protocol Compatibility](docs/protocol-compatibility.md)
...
- 图片/多模态能力取决于上游模型是否支持对应 Chat Completions 内容格式。默认不转发图片；如使用视觉模型，请在 Web 管理台预设中勾选“支持图片输入”或设置 `upstream_supports_image_input=true`。
...
- Converts text and optional image inputs into Chat Completions compatible payloads; text-only models reject images with a clear error unless image support is enabled in the preset.
...
- [Responses Protocol Compatibility](docs/protocol-compatibility.md)
...
- Image/multimodal support depends on whether the upstream model supports the translated Chat Completions content format. Images are not forwarded by default; enable "supports image input" in the Web Manager preset or set `upstream_supports_image_input=true` for vision models.
